...
public interface IDungeonTraitEffect
{
    ...
    /// </summary>
    public void OnDungeonDiscovered(Grade grade);

    /// <summary>
    /// 던전 유지 시간 초과 시 적용되는 특성 EX)영지에 침공하는 몬스터 수 증가
    /// </summary>
    public void OnDungeonOverflowed(Grade grade);

    /// <summary>
    /// 모험단 던전 공략 위해 배치 시 적용되는 특성 EX) 특정 직업의 모험가가 존재하지 않는경우 던전 공략 실패
    /// </summary>
    public void OnAdventureAssigned(Grade grade);
    ...
    /// </summary>
    public void OnCombatFailed(Grade grade);

    /// <summary>
    /// 던전 공략 성공 시 적용되는 특성 EX) 하위던전 여러개 추가 발생
    /// </summary>
    public void OnCombatSucceeded(Grade grade);
